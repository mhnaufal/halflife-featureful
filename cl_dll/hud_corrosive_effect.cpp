...
DECLARE_MESSAGE(m_CorrosiveEffect, CorEffect)

int CHudCorrosiveEffect::Init(void)
{
	// ConsolePrint("ASU 1\n");
	gHUD.AddHudElem(this);
	HOOK_MESSAGE(CorEffect);
...
int CHudCorrosiveEffect::VidInit(void)
{
	m_hSprite = SPR_Load("sprites/custom/poison_effect.spr");
...
void CHudCorrosiveEffect::Reset(void)
{
	m_flEffectEnd = 0;
	m_flIntensity = 0;
	m_iFlags = 0;
...
// TODO: draw sprite to left and right camera
int CHudCorrosiveEffect::Draw(float flTime)
{
...
void CHudCorrosiveEffect::ShowEffect(long duration, long intensity)
{
...
int CHudCorrosiveEffect::MsgFunc_CorEffect(const char* pszName, int iSize, void* pbuf)
{
